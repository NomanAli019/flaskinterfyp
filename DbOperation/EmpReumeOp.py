...
class EmployeeResumesOperations:
    ...
    def get_resumes_by_employee(employee_id):
        with app.app_context():
            resumes = EmployeeResumes.query.filter_by(employee_id=employee_id).all()
            return [{"id": resume.employee_res_id, "employee_id": resume.employee_id, "path": resume.employee_resume_path} for resume in resumes] if resumes else None
